...
# Maximum APK size: 200 MB
MAX_APK_SIZE = 200 * 1024 * 1024

# CORS configuration - restrict origins for security
ALLOWED_ORIGINS = os.environ.get("ALLOWED_ORIGINS", "http://localhost:3000,http://10.0.2.2:8000").split(",")
app.add_middleware(
    CORSMiddleware,
    ...
    **Privacy Note**: The APK is processed in memory and not permanently stored.
    ...
    try:
        ...
        if len(apk_bytes) > MAX_APK_SIZE:
            raise HTTPException(
                status_code=413,
                detail=f"APK file too large. Maximum size: {MAX_APK_SIZE // (1024*1024)} MB"
            )
...
async def _perform_analysis(apk_bytes: bytes, file_hash: str) -> AnalysisResponse:
    ...
    # Parse APK (CPU-bound, run in thread pool)
    metadata = await asyncio.to_thread(parser.parse_from_bytes, apk_bytes)
    
    # Run permission analysis (CPU-bound, run in thread pool)
    perm_result = await asyncio.to_thread(permission_analyzer.analyze, metadata.permissions)
    
    # Run certificate analysis (CPU-bound, run in thread pool)
    cert_result = await asyncio.to_thread(certificate_analyzer.analyze, metadata.certificate_info)
    
    # Run obfuscation detection (CPU-bound, run in thread pool)
    dex_info = await asyncio.to_thread(parser.get_dex_info)
    obf_result = await asyncio.to_thread(obfuscation_detector.analyze, dex_info)
    
    # Run network extraction (CPU-bound, run in thread pool)
    net_result = await asyncio.to_thread(network_extractor.analyze, strings=dex_info.get("classes", []))
...
def _build_report_data(analysis: AnalysisResponse) -> ReportData:
    """Build ReportData from an AnalysisResponse — shared by both PDF endpoints."""
    return ReportData(
...
async def generate_pdf_report(analysis: AnalysisResponse):
    ...
    """
    try:
        report_data = _build_report_data(analysis)

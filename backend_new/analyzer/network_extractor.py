...
class NetworkExtractor:
    ...
    def _analyze_endpoint(self, url: str) -> Optional[NetworkEndpoint]:
        """Analyze a single endpoint. Returns None if URL cannot be parsed."""
        try:
            parsed = urlparse(url)
            ...
            # Log parse failure for debugging rather than silently swallowing
            print(f"WARNING: Failed to parse endpoint URL '{url}': {e}")
            return None
